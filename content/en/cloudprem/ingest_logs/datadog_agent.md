...
In case you are configure the Datadog agent to use a proxy and if CloudPrem is hosted in your internal network, you need to configure the `no_proxy` agent so it can sends logs directly to CloudPrem without passing through the proxy.
...
Additionally, you need to set `DD_NO_PROXY_NONEXACT_MATCH` to true. You can read more details in the [agent proxy documentation](https://docs.datadoghq.com/agent/configuration/proxy/#proxy-server-setup-examples).
